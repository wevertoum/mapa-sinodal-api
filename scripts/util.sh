...
get_project_id() {
  ...
  if [[ -z "$project" ]]; then
    echo >&2 "No default project was found, and DEVSHELL_PROJECT_ID is not set."
    echo >&2 "Please use the Cloud Shell or set your default project by typing:"
    echo >&2 "gcloud config set project YOUR-PROJECT-NAME"
  fi
  echo "$project"
}
